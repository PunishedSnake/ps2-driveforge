    ...
    std::string_view csv, std::string_view canonical_game_id)
{
    ...
    while (cursor <= csv.size()) {
        ...
        if (cursor == 0 && line.size() >= 3 &&
            ...
            static_cast<unsigned char>(line[2]) == 0xbf) line.remove_prefix(3);
...
[[nodiscard]] inline GameMetadata parse_cfg_metadata(std::string_view text)
{
    ...
    while (cursor <= text.size()) {
        ...
        if (!clean.empty() && clean.front() != '#' && clean.front() != ';') {
            ...
            if (equals != std::string::npos) metadata_detail::assign_cfg_field(result, clean.substr(0, equals), clean.substr(equals + 1));
        }
        if (end == std::string_view::npos) break;
        cursor = end + 1;
    }
    ...
    std::string fallback_title = {})
{
    ...
    for (const auto& asset : assets) {
        if (asset.kind == AssetKind::title_database) {
            if (const auto text = metadata_detail::read_text_bounded(asset.staged_path)) database_title = lookup_title_database(*text, canonical_game_id);
