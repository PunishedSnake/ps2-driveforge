...
struct WritableFileBlockDevice::Impl {
#ifdef _WIN32
...
WritableFileBlockDevice::WritableFileBlockDevice(std::filesystem::path path)
    : path_(std::move(path)), impl_(std::make_unique<Impl>())
{
#ifdef _WIN32
    impl_->handle = CreateFileW(path_.c_str(), GENERIC_READ | GENERIC_WRITE,
                                FILE_SHARE_READ,
...
#else
    impl_->fd = ::open(path_.c_str(), O_RDWR);
    if (impl_->fd < 0) {
        return;
    }
...
WritableFileBlockDevice::~WritableFileBlockDevice()
{
    if (!impl_) {
        return;
    }
...
bool WritableFileBlockDevice::is_open() const noexcept
{
    if (!impl_) {
        return false;
    }
...
std::uint64_t WritableFileBlockDevice::size_bytes() const
{
    return size_;
}

std::string WritableFileBlockDevice::display_name() const
{
    return path_.string();
}

bool WritableFileBlockDevice::read(std::uint64_t offset, std::span<std::byte> out)
{
...
bool WritableFileBlockDevice::write(std::uint64_t offset, std::span<const std::byte> in)
{
    ...
    while (done < in.size()) {
        const DWORD chunk = static_cast<DWORD>(
            std::min<std::size_t>(in.size() - done, std::numeric_limits<DWORD>::max()));
        if (!ResetEvent(event.get())) {
            return false;
        }
        ...
        overlapped.hEvent = event.get();
        set_overlapped_offset(overlapped, offset + done);
        const BOOL started = WriteFile(impl_->handle, in.data() + done, chunk, nullptr, &overlapped);
        DWORD transferred = 0;
        if (!finish_overlapped(impl_->handle, overlapped, started, transferred) || transferred == 0) {
            return false;
        }
    ...
    while (done < in.size()) {
        ...
        const auto chunk = std::min<std::size_t>(
            in.size() - done, static_cast<std::size_t>(std::numeric_limits<ssize_t>::max()));
        const ssize_t transferred = ::pwrite(impl_->fd, in.data() + done, chunk,
                                             static_cast<off_t>(absolute));
        if (transferred <= 0) {
            return false;
        }
...
bool WritableFileBlockDevice::flush()
{
    if (!is_open()) {
        return false;
    }
#ifdef _WIN32
    return FlushFileBuffers(impl_->handle) != FALSE;
#else
    return ::fsync(impl_->fd) == 0;
#endif
}
